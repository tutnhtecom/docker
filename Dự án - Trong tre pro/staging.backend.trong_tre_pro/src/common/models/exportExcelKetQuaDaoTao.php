...
class exportExcelKetQuaDaoTao
{
    public $data;
    public $path_file;
    public $creator = 'TRONG TRE PRO SETE CO.,Ltd';
    public $title = 'Báo cáo ket qua dao tao';
    public $subject = 'Báo cáo ket qua dao tao';
    public $description = '';
    public $category = '';
    public $keywords = '';
    public $sheetTitle = 'bao cao ket qua dao tao';
    public $legal = 'bao cao ket qua dao tao';
    ...
    public function renderBody($activeSheet){
        ...
        foreach ($data['data'] as $item) {
            $activeSheet
                ->setCellValue("A{$dong}",$item->hocPhan)
                ->setCellValue("B{$dong}",date('d/m/Y', strtotime($item->created)))
                ->setCellValue("C{$dong}", $item->hoten==""?$item->dien_thoai:$item->hoten)
                ->setCellValue("D{$dong}", $item->baiHoc)
                ->setCellValue("E{$dong}", $item->capDo)
                ->setCellValue("F{$dong}", $item->trang_thai)
            ;
            $dong++;
        }
        $dong--;
        formatExcel::setBorder($activeSheet, "A5:F{$dong}");
        $activeSheet
    ...
    public function run()
    {
        $objPHPExcel = \PhpOffice\PhpSpreadsheet\IOFactory::load(dirname(dirname(__DIR__)).'/common/template/BaoCaoKetQuaDaoTao.xlsx');
        $activeSheet = $objPHPExcel->getActiveSheet();
        $this->renderBody($activeSheet);
        $this->filename = 'BaoCaoKetQuaDaoTao_'.date('d_m_Y__H_i_s').'.xlsx';
